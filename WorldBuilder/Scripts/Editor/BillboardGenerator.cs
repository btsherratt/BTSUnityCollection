...
namespace SKFX.WorldBuilder {
    public static class BillboardGenerator {
        ...
        [MenuItem("Assets/SKFX/Generate Billboards", false, 100)]
        static void GenerateBillboards() {
            foreach (GameObject gameObject in Selection.gameObjects) {
                if (gameObject.GetComponentInChildren<Renderer>() != null) {
                    GenerateBillboard(gameObject);
                }
            }
        }

        [MenuItem("Assets/SKFX/Generate Billboards", true)]
        ...
        static void GenerateBillboard(GameObject gameObject) {
